...
class AnkiLogic:
    ...
    def create_grammar_model(self, id: int):
        """建立 grammar model"""
        return genanki.Model(
            id,
            f"{self.deck_name}_grammar",
            fields=GRAMMAR_FIELDS,
            templates=BASIC_GRAMMAR_TEMPLATES,
            css=BASIC_CSS
        )
    ...
    def create_grammar_note(
        ...
        # 確保所有字段都是字符串，將 None 轉換為空字符串
        fields = [
            str(grammar) if grammar is not None else "",
            str(usage) if usage is not None else "",
            str(meaning) if meaning is not None else "",
            str(contrast) if contrast is not None else "",
            str(ex1_ori) if ex1_ori is not None else "",
            str(ex1_trans) if ex1_trans is not None else "",
            str(ex2_ori) if ex2_ori is not None else "",
            str(ex2_trans) if ex2_trans is not None else "",
    ...
    def get_or_create_grammar_model(self):
        """
        建立新的 grammar model（已移除本地数据库连接，总是创建新 model）。
        """
        model_name = f"{self.deck_name}_grammar"
        model_id = self.random_model_id()
        logger.log(LogLevel.INFO, f"創建新的 Grammar model '{model_name}' (ID: {model_id})")
        return self.create_grammar_model(model_id)
