...
class GPTClient:
    ...
    def passage_with_question(self, passage_image_folder: str = None, question: str = PROMPT_EN_PASSAGE_VOCAB_QUESTIONS, image_paths: list[str] = None):
        ...
        Args:
            ...
            question: 問題提示
            image_paths: 指定的圖片路徑列表（優先使用，如果提供則忽略 passage_image_folder）
        """
        logger.log(LogLevel.INFO, f"收集圖片中...")
        logger.log(LogLevel.DEBUG, f"passage_image_folder: {passage_image_folder}, image_paths: {image_paths}")
        ...
        if image_paths is None:
            ...
            logger.log(LogLevel.DEBUG, f"收集到 {len(image_paths)} 張圖片")
