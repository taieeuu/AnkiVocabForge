...
async def package_cards(data: Dict[str, Any]):
    ...
    try:
        ...
        # 判斷卡片類型：grammar 還是 vocab
        # 通過檢查第一個卡片是否有 'grammar' 字段來判斷
        is_grammar = len(cards) > 0 and 'grammar' in cards[0] and cards[0].get('grammar')
        
        # 確定卡片類型（用於 vocab 的 Basic/Cloze 判斷）
        ...
        if is_grammar:
            ...
            logger.info(f"Saved edited grammar JSON to: {json_path}")
            
            # Grammar 卡片使用 import_grammar_model_notes（不需要 voice_dir）
            result = AnkiService.import_grammar_model_notes(
                grammar_list, deck_name, 
            ...
            logger.info(f"Saved edited vocab JSON to: {json_path}")
