...
def parse_request_data(data: Dict[str, Any]) -> dict:
    ...
    # 處理文法列表（類似 vocabList 的處理方式）
    # 支持 grammarList (對象格式) 和 grammarFile (字符串格式，來自前端)
    grammar_list_data = data.get('grammarList', '')
    grammar_file_data = data.get('grammarFile', '')  # 前端可能使用 grammarFile
    
    if isinstance(grammar_list_data, dict):
        grammar_list = grammar_list_data.get('content', '')
        grammar_file_name = grammar_list_data.get('filename', '')
    elif grammar_file_data:
        # 前端使用 grammarFile 字段（字符串格式）
        grammar_list = grammar_file_data
        grammar_file_name = data.get('grammarFileName', 'grammar_list.txt')
    else:
        # 向後兼容：舊格式直接是字符串
        grammar_list = grammar_list_data
        grammar_file_name = data.get('grammarFileName', '')
    ...
    return {
        ...
        'content_type': data.get('contentType', 'Word')  # 新增：支持選擇 Word 或 Grammar
    }
...
def load_generated_cards(orig_dir: Path) -> List[Dict]:
    ...
    with open(json_files[0], 'r', encoding='utf-8') as f:
        ...
        # 轉換為前端需要的格式
        cards = []
        # 檢查第一個項目是否有 grammar 欄位來判斷是文法還是單字
        is_grammar = len(cards_data) > 0 and 'grammar' in cards_data[0]
