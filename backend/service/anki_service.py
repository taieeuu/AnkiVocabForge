...
class AnkiService:
    ...
    @staticmethod
    def import_grammar_model_notes(grammar_list: list[dict], deck_name: str = "MyTestDeck", session_dir: str = None, filename_suffix: str = None) -> str:
        """
        匯入 Grammar 模型的 notes 到 Anki deck
        
        Args:
            grammar_list: 文法列表，每個元素為包含文法資訊的字典
            deck_name: deck 名稱
            session_dir: 輸出目錄
            filename_suffix: 檔案名稱後綴
        ...
        # 建立 Grammar model 並產生 notes
        logger.log(LogLevel.INFO, "建立 Anki Grammar model 與 notes...")
        model = logic.get_or_create_grammar_model()
        
        for g in grammar_list:
            note = logic.create_grammar_note(
                model=model,
                grammar=g.get("grammar") or "",
                usage=g.get("usage") or "",
                meaning=g.get("meaning") or "",
                contrast=g.get("contrast") or "",
                ex1_ori=g.get("ex1_ori") or "",
                ex1_trans=g.get("ex1_trans") or "",
                ex2_ori=g.get("ex2_ori") or "",
                ex2_trans=g.get("ex2_trans") or "",
            )
            logic.create_anki_card(note)
        logger.log(LogLevel.INFO, f"✅ 已建立 {len(grammar_list)} 個 Grammar notes")
        ...
        if filename_suffix:
            ...
            apkg_filename = f'{safe_deck_name}.apkg'
        logger.log(LogLevel.INFO, f"✅ 打包完成：{apkg_filename}")
        return f"打包完成，請在 Anki 中匯入 {apkg_filename}"
